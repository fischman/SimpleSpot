...
test('addToQueue with toFront prepends URIs', async () => {
  ...
  mockApiRoute(/\/tracks/, { tracks: [FIXTURES.track, FIXTURES.track2, FIXTURES.track3] });
...
test('queueTrackMapper includes drag handle', () => {
  ...
  ok(result.suffix && result.suffix.includes('drag-handle'), 'should have drag handle');
});

test('queueTrackMapper includes remove button in queueBtn', () => {
  const result = queueTrackMapper(FIXTURES.track, 0, 1);
  ok(result.queueBtn && result.queueBtn.includes('removeFromQueue'), 'should have remove button in queueBtn');
});
...
test('loop re-adds track on next()', async () => {
  player = new Spotify.Player({});
  player._state = { paused: false, context: {metadata:{}}, track_window: { current_track: FIXTURES.track } };
...
test('previous restores from history', async () => {
  player = new Spotify.Player({});
  player._state = { paused: false, context: {metadata:{}}, track_window: { current_track: FIXTURES.track2 } };
